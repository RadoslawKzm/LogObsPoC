...
from backend.api.loguru_logger import loguru_envs
...
def logger_setup():
    ...
    logger.level("END 200", no=18, color="<underline><bold><green>", icon="✅")
    logger.level("END 300", no=19, color=f"<underline><bold>{WARM_ORANGE}", icon="🔀")
    logger.level("END 400", no=27, color=f"<underline><bold>{ORANGE_RED}", icon="⚠️")
    logger.level("END 500", no=28, color=f"<underline><bold>{BOLD_RED}", icon="💥")
