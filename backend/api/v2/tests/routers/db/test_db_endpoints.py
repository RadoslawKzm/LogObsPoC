...
# --- Tests ---
@patch("backend.api.database.PostgresImplementation.get_record", new_callable=AsyncMock)
@pytest.mark.asyncio
...
@patch(
    "backend.api.database.MongoImplementation.get_many_records", new_callable=AsyncMock
)
@pytest.mark.asyncio
...
# @patch("backend.api.database.MockImplementation.get_many_records", new_callable=AsyncMock)
@pytest.mark.asyncio
async def test_mock_endpoint(client, app):
    # mock_get_many_records.return_value = [{"id": 42}]
