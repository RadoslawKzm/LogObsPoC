...
class ErrorDoc(BaseModel):
    internal_code: int = Field(
        ..., description="Internal code used for tracking"
    )
    http_code: int = Field(..., description="HTTP response code")
    internal_message: str = Field(
        ..., description="Detailed internal log message"
    )
    ...
    module: str = Field(
        ..., description="Module in which the error is defined"
    )
    members: dict[str, "ErrorDoc"] | None = Field(
...
#         if issubclass(base, base_exceptions.BaseCustomError)
#         and base is not base_exceptions.BaseCustomError
#     ][
#         ::-1
#     ]  # reverse to get from top to bottom
#
#     # Root of tree
...
#     if internal_code in seen_internal_codes:
#         raise ValueError(
#             f"Duplicate internal code: {internal_code} in {exception_cls.__name__}"
#         )
#     seen_internal_codes.add(internal_code)
#
