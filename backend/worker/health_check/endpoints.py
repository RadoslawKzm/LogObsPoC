...
PREVIOUS_STATE: dict = {
    "time": time.time(),
    "usage_ns": get_cgroup_usage(path=worker_settings.CGROUP_CPU_USAGE),
}
...
def container_cpu_usage() -> tuple:
    ...
    cpu_quota = get_cgroup_usage(path=worker_settings.CGROUP_CPU_QUOTA)
    cpu_period = get_cgroup_usage(path=worker_settings.CGROUP_CPU_PERIOD)
...
def container_ram_usage():
    ram_usage = get_cgroup_usage(path=worker_settings.CGROUP_MEMORY_USAGE)
    total_ram = get_cgroup_usage(path=worker_settings.CGROUP_MEMORY_LIMIT)
...
def is_container():
    try:
        container = get_cgroup_usage(path=worker_settings.CGROUP_CPU_USAGE)
        if not container:
            return False
        return True
    except Exception as e:
