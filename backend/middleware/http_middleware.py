...
#         # Replace the stream so downstream handlers can still read it
#         async def receive():
#             return {"type": "http.request", "body": body, "more_body": False}
#         request._receive = receive
#         request.state.body = body  # stored but unused unless needed
#
#         http_version = request.scope.get("http_version")
#         with logger.contextualize(
...
#         )
#
#         # Log differently per status code
#         if 200 <= response.status_code <= 299:
#             logger.log("EXIT 200", msg)
#         elif 300 <= response.status_code <= 399:
#             logger.log("EXIT 300", msg)
...
#                 logger.warning(
#                     f"Validation error: {msg} | BODY={request.state.body.decode(errors='ignore')}"
#                 )
#             else:
#                 logger.log("EXIT 400", msg)
#         else:  # 500++
